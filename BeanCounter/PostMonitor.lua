...
-- We only need to know if this is Classic Era, for different Auction Duration times
lib.isClassicEra = WOW_PROJECT_ID == WOW_PROJECT_CLASSIC
	...
	if (name and count) then
		...
		for bagID = 0, 4 do
			...
			for  slot = 1, bagSlots do
				local  _, selectedStack, locked, _, _ = GetContainerItemInfoSpecial(bagID, slot)
				if locked then
...
function private.bagSnapshot( itemBeingPosted, totalQuanity)
	...
	for bagID = 0, 4 do
		...
		for slot = 1, slots do
			local link = GetContainerItemLink(bagID, slot)
			local texture, itemCount, locked, quality, readable = GetContainerItemInfoSpecial(bagID, slot)
