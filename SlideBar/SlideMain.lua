...
-- Name of the config panel as displayed in the config menu
local PANEL_NAME = "Norganna SlideBar"
...
-- LibDataBroker is normally packaged with SlideBar in Release versions
-- Also the TOC will try to load standalone LibDataBroker first, if it exists
	...
	frame:SetScript("OnEvent", function(self, event, arg, ...)
		...
		elseif event == "ADDON_LOADED" and arg == "SlideBar" then
			if type(SlideBarConfig) == "table" then
				...
				if WOW_PROJECT_ID == WOW_PROJECT_MAINLINE then  -- addon compartment optional checking
					if SlideBarConfig.baoc == nil then
						SlideBarConfig.baoc = true  -- addon compartment optional checking
					else  -- addon compartment optional checking
						SlideBarConfig.baoc = toboolean(SlideBarConfig.baoc)  -- addon compartment optional checking
					end
				end  -- addon compartment optional checking
...
function private.CommandHandler(msg)
	...
	if (a == "config") then
		Settings.OpenToCategory(frame.config.ID)
	end
	if (save) then
		lib.ApplyLayout()
	end
...
function private.GUI()
	...
	config:Hide()
	config.name = PANEL_NAME
	config.ID = PANEL_NAME -- use PANEL_NAME as the ID
	...
	-- Begin: AddOnCompartment... Quick Access to Settings
	if AddonCompartmentFrame and SlideBarConfig.baoc then
		local mouseButtonNote = "\nA SlideBar for Addon UI Shortcuts.";
		AddonCompartmentFrame:RegisterAddon({
			text = config.name,
			-- icon = "Interface/AddOns/SlideBar/noIcon.blp",
			notCheckable = true,
			func = function(button, menuInputData, menu)
				Settings.OpenToCategory(config.name)
			end,
			funcOnEnter = function(button)
				MenuUtil.ShowTooltip(button, function(tooltip)
					tooltip:SetText(config.name .. mouseButtonNote)
				end)
			end,
	...
	-- End: AddOnCompartment... Quick Access to Settings
	...
	-- create the addons compartment option
	if WOW_PROJECT_ID == WOW_PROJECT_MAINLINE then
		local enableBAOC = CreateCheckButton("baoc", "Enable Blizzard AddOns Compartment (Changes to this take effect next UI Reload)")
		config.enableBAOC = enableBAOC
		enableBAOC:SetPoint("LEFT", frame.config, "LEFT", 10, -50)
	end
	-- create the addons compartment option
