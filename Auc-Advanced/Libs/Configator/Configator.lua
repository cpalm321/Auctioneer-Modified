...
local LIBRARY_VERSION_MINOR = 38
...
function kit:KeyPress(key, ...)
	...
	if self.subcontrol and self.subcontrol.hasFocus then
		local script = self.subcontrol:GetScript("OnKeyUp")
		if script then
			script(key, ...)
		end
	end
...
function kit:AddControl(id, cType, column, ...)
	...
	or cType == "NumeriSlider" or cType == "NumeriWide" or cType == "NumeriTiny") then
		...
		-- Slider
		local tmpName = lib.CreateAnonName()
		el = CreateSlider(tmpName, content)	-- ### hybrid
		el:SetSize(144, 17)
		...
		if hasNumber then
			local tmpName2 = lib.CreateAnonName()
			subcon = CreateFrame("EditBox", tmpName2, el, "InputBoxTemplate")
			lib:TabLink(frame, subcon)

			subcon:SetPoint("LEFT", el, "RIGHT", 8,0)
			subcon:SetWidth(40)
			subcon:SetHeight(32)
			subcon:SetScale(0.8)
			subcon:SetScript("OnEditFocusGained", function(...)
				self.curFocus = subcon
				subcon.hasFocus = true
			end)
			subcon:SetScript("OnEditFocusLost", function(...)
				local myMin, myMax = subcon.minValue, subcon.maxValue
				local myVal = math.min(myMax, math.max(myMin, tonumber(subcon:GetNumber()) or 0))
				if (el:GetValue() ~= myVal*100) then
					el:SetValue(myVal*100)
				end
				subcon:SetNumber((el:GetValue())/100)
				subcon.hasFocus = false
				self.curFocus = nil
			end)
			subcon:SetScript("OnEscapePressed", kit.Unfocus)
			subcon:SetScript("OnEnterPressed", kit.Unfocus)
			subcon:SetAutoFocus(false)
			subcon:Show()
			subcon.minValue = min
			subcon.maxValue = max
			subcon.element = el
			subcon:SetNumber((el:GetValue())/100)
			el.subcontrol = subcon
		end
		el:SetScript("OnValueChanged", function(element, value)
			self:ChangeSetting(element, value)
			if (subcon) then
				local myVal = el:GetValue()
				if subcon:GetNumber() ~= myVal/100 then
					subcon:SetNumber(myVal/100)
				end
			end
		end)
		self.elements[setting] = el
