...
local settingDefaults = {
	...
	["columnwidth.ilvl"] = 40,
	["columnwidth.Stam"] = 40,
	["columnwidth.Spirit"] = 40,
	["columnwidth.Int"] = 40,    -- Using probable key, verify if needed
	["columnwidth.Str"] = 40,
	["columnwidth.Agi"] = 40,
	["columnwidth.Armor"] = 50,
...
-- Around line 640, function private.removeline(force)
function private.removeline(force)
	local selectedRowIndexInData = gui.sheet.selected
	local sortListIndex = -1

	for i,j in ipairs(gui.sheet.sort) do
		if j == selectedRowIndexInData then
			sortListIndex = i
			break
		end
	end
	...
	if force or not lib.GetSetting("global.fast_remove_mode") then
		if selectedRowIndexInData then
			tremove(private.sheetData, selectedRowIndexInData)
			-- if private.sheetReasons then tremove(private.sheetReasons, selectedRowIndexInData) end -- Remove old line if it exists
            if private.sheetAuxData then tremove(private.sheetAuxData, selectedRowIndexInData) end -- <<< UPDATE THIS

			local total =  #private.sheetData
			for i = selectedRowIndexInData, total do
				private.sheetStyle[i] = private.sheetStyle[i+1]
			end
			private.sheetStyle[total+1] = nil
		end
	else
		if sortListIndex ~= -1 and sortListIndex < #gui.sheet.sort then
			sortListIndex = sortListIndex + 1
		else
			sortListIndex = #gui.sheet.sort
		end
	end
	...
	if sortListIndex ~= -1 and gui.sheet.sort[sortListIndex] then
		gui.sheet.selected = gui.sheet.sort[sortListIndex]
	elseif #gui.sheet.sort > 0 then
...
-- Around line 670, function private.removeall()
function private.removeall()
	empty(private.sheetData)
	empty(private.sheetStyle)
    -- if private.sheetReasons then empty(private.sheetReasons) end -- Remove old line if it exists
    if private.sheetAuxData then empty(private.sheetAuxData) end   -- <<< UPDATE THIS
	gui.sheet.selected = nil
	gui.sheet:SetData(private.sheetData, private.sheetStyle)
	gui.sheet:Render()
	lib.UpdateControls()
...
function private.purchaseall()
	...
	-- Around line 755, function private.purchaseall()
-- ...
	for _, sortedRowIndex in ipairs(gui.sheet.sort) do
		local bidqueue = gui.frame.cancel.value or 0
		local balance = money - bidqueue

		local dataFromSheet = gui.sheet:GetRowData(sortedRowIndex) -- `dataFromSheet` is an 8-element row
        -- Update indices based on the new 8-column structure
		local link = dataFromSheet[1]         -- Was 1 -> still 1
		local stack = dataFromSheet[2]        -- Was dataFromSheet[4] in 10-col -> now 2
		local buyout_price = dataFromSheet[3] -- Was dataFromSheet[5] in 10-col -> now 3
		local bid_price = dataFromSheet[4]    -- Was dataFromSheet[6] in 10-col -> now 4
		local seller = dataFromSheet[5]       -- Was dataFromSheet[7] in 10-col -> now 5

        -- Fetch reason and minbid from our auxiliary data table (this part is unchanged)
		local reasonString
		local minbid_val = 0
		if private.sheetAuxData and private.sheetAuxData[sortedRowIndex] then
			local auxData = private.sheetAuxData[sortedRowIndex]
			reasonString = auxData.reason
			minbid_val = auxData.minbid
		else
			reasonString = nil
			minbid_val = 0
		end
		...
		local priceToPay = 0
		if reasonString and strmatch(reasonString, ":buy") then
			priceToPay = buyout_price
		elseif reasonString and strmatch(reasonString, ":bid") then
			priceToPay = bid_price
		elseif buyout_price > 0 then
			priceToPay = buyout_price
		else
			priceToPay = bid_price
		end

		if ((balance-priceToPay) > reserve or not enableres) then
			AucAdvanced.Buy.QueueBuy(link, seller, stack, minbid_val, buyout_price, priceToPay, private.cropreason(reasonString))
		else
			aucPrint("Purchase cancelled: Reserve reached")
		end
	end
...
function private.MakeGuiConfig()
	...
	function lib.UpdateControls()
		...
		-- Around line 974, function lib.UpdateControls()
-- ...
		if selected ~= gui.sheet.selected then
			selected = gui.sheet.selected
			local dataFromSheet = gui.sheet:GetSelection() -- This `dataFromSheet` is now an 8-element array
			if not dataFromSheet then
				empty(private.data)
			else
                -- Update indices based on the new 8-column structure
				private.data.link = dataFromSheet[1]       -- Was 1 -> still 1
				private.data.stack = dataFromSheet[2]      -- Was dataFromSheet[4] in 10-col -> now 2
				private.data.buyout = dataFromSheet[3]     -- Was dataFromSheet[5] in 10-col -> now 3
				private.data.bid = dataFromSheet[4]        -- Was dataFromSheet[6] in 10-col -> now 4 (this is item[Const.PRICE])
				private.data.seller = dataFromSheet[5]     -- Was dataFromSheet[7] in 10-col -> now 5

                -- Fetch reason, minbid, curbid from private.sheetAuxData (this part is unchanged)
				if private.sheetAuxData and gui.sheet.selected and private.sheetAuxData[gui.sheet.selected] then
					local auxData = private.sheetAuxData[gui.sheet.selected]
					private.data.reason = auxData.reason
					private.data.minbid = auxData.minbid
					private.data.curbid = auxData.curbid
				else
					private.data.reason = nil
					private.data.minbid = 0
					private.data.curbid = 0
				end
			end
-- ... (rest of the function)
	...
	function lib.OnEnterSheet(button, row, index)
		if gui.sheet.rows[row][index]:IsShown() then --Hide tooltip for hidden cells
			...
			if linkType then
				local count
				-- Around line 1000, function lib.OnEnterSheet(button, row, index)
-- ...
				if gui.sheet.order then
                    ...
                    -- Stk was originally index 4, then remained 4 in 10-col, now it's index 2 in 8-col
					count = tonumber(gui.sheet.rows[row][2]:GetText() ) -- <<< UPDATE index to 2
				end
-- ...
	...
	gui.sheet = ScrollSheet:Create(gui.frame, {
		...
		{ "Str",    "INT", lib.GetSetting("columnwidth.Str"), { DESCENDING=true } },     -- 14
		{ "Agi",    "INT", lib.GetSetting("columnwidth.Agi"), { DESCENDING=true } },     -- 15
		{ "Armor",  "INT", lib.GetSetting("columnwidth.Armor"), { DESCENDING=true } },   -- 16
	...
	function gui.sheet.Processor(callback, self, button, column, row, order, curDir, ...)
		...
		elseif (callback == "ColumnWidthReset") then
			private.OnResizeSheet(self, column, nil)
	...
	gui.Search = CreateFrame("Button", "AucSearchUISearchButton", gui, "UIPanelButtonTemplate")
	gui.Search:SetSize(90, 21)
	...
	gui.frame.purchase = CreateFrame("Button", nil, gui.frame, "UIPanelButtonTemplate")
	gui.frame.purchase:SetSize(90, 21)
	...
	gui.frame.notnow = CreateFrame("Button", nil, gui.frame, "UIPanelButtonTemplate")
	gui.frame.notnow:SetSize(90, 21)
	...
	gui.frame.ignore = CreateFrame("Button", nil, gui.frame, "UIPanelButtonTemplate")
	gui.frame.ignore:SetSize(90, 21)
	...
	gui.frame.ignoreperm = CreateFrame("Button", nil, gui.frame, "UIPanelButtonTemplate")
	gui.frame.ignoreperm:SetSize(90, 21)
	...
	gui.frame.snatch = CreateFrame("Button", nil, gui.frame, "UIPanelButtonTemplate")
	gui.frame.snatch:SetSize(90, 21)
	...
	gui.frame.cancel:SetSize(22, 18)
	...
	gui.frame.buyout:SetSize(90, 21)
	...
	do -- bid button and bid box
		local bidbutton = CreateFrame("Button", nil, gui.frame, "UIPanelButtonTemplate")
		gui.frame.bid = bidbutton
		bidbutton:SetSize(90, 21)
	...
	gui.frame.progressbar.cancel = CreateFrame("Button", nil, gui.frame.progressbar, "UIPanelButtonTemplate")
...
function lib.SearchItem(searcherName, item, nodupes, skipresults)
	...
	if buyorbid then
		...
		local enableres = lib.GetSetting("reserve.enable")
		local reserve = lib.GetSetting("reserve") or 1
		local bidqueue = (gui and gui.frame and gui.frame.cancel and gui.frame.cancel.value) or 0
