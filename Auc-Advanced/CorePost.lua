...
local GetContainerItemDurability = C_Container and C_Container.GetContainerItemDurability or GetContainerItemDurability
...
local ErrorText = {
	...
	APINotLoaded = "The AuctionHouse is not loaded",
	MaximumBid = "Bid price is more than the maximum allowed",
}
...
if AucAdvanced.ABORTLOAD then
	...
	function private.GetRequest(item, size, bid, buyout, duration, multiple)
		...
		elseif not MAXIMUM_BID_PRICE then
			return nil, "APINotLoaded" -- Blizzard_AuctionUI not loaded, we need this to check max bid price
		elseif bid > MAXIMUM_BID_PRICE then
			return nil, "MaximumBid"
...
function lib.PostAuctionClick(sig, size, bid, buyout, duration, multiple)
	...
	if postNow and AuctionFrame and AuctionFrame:IsVisible() then
...
function lib.IsAuctionable(bag, slot)
	...
	local _,_,_,_,_,lootable = GetContainerItemInfoSpecial(bag, slot)
	if lootable then
	...
	for bag = 0, NUM_BAG_FRAMES do
		for slot = 1, GetContainerNumSlots(bag) do
			...
			if link and GetSigFromLink(link) == sig and lib.IsAuctionable(bag, slot) then
				local _, thiscount, locked = GetContainerItemInfoSpecial(bag,slot)
				if locked then
...
private.Prompt.Yes = CreateFrame("Button", "AuctioneerPostPromptYes", private.Prompt, "UIPanelButtonTemplate")
private.Prompt.Yes:SetSize(90, 21)
...
private.Prompt.No = CreateFrame("Button", "AuctioneerPostPromptNo", private.Prompt, "UIPanelButtonTemplate")
private.Prompt.No:SetSize(90, 21)
