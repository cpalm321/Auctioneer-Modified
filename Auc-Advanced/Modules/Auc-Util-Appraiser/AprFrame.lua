...
-- In this file we only need up to the 5th return value; however this is copied from CorePost which uses more values
...
local CalcPriceLevel -- to be filled in when PriceLevel loads
AucAdvanced.RegisterModuleCallback("pricelevel", function(lib) CalcPriceLevel = lib.CalcLevel end)
local IsPlayerIgnored, PromptSellerIgnore -- to be filled in when Filter-Basic loads
AucAdvanced.RegisterModuleCallback("basic", function(lib) IsPlayerIgnored = lib.IsPlayerIgnored; PromptSellerIgnore = lib.PromptSellerIgnore end)
...
function private.CreateFrames()
			...
			for slot=1,GetContainerNumSlots(bag) do
				...
				if link then
					...
					if AucAdvanced.Post.IsAuctionable(bag, slot) or isDirect then
						...
						if sig then
							local texture, itemCount, locked, special, readable = GetContainerItemInfoSpecial(bag,slot)
	...
	function frame.DisplaySelectedItem(item, sig, pos)
		...
		if not item[3] then
			frame.salebox.icon:ClearNormalTexture()
		else
	...
	function private.DelayedImageUpdate()
		...
		for i = 1, #results do
			...
			if IsPlayerIgnored and IsPlayerIgnored(result[Const.SELLER]) then
	...
	function frame.SetPriceColor(link, count, requiredBid, buyoutPrice, rDef, gDef, bDef)
		if CalcPriceLevel and get('util.appraiser.color') then
			if type(link) == "number" then
				local _, l = GetItemInfo(link)
				link = l
			end
			if not link then return end
			local _, _, r,g,b = CalcPriceLevel(link, count, requiredBid, buyoutPrice)
			r, g, b = r or rDef, g or gDef, b or bDef
	...
	function frame.ShowOwnAuctionDetails(itemLink)
        ...
        local colored = CalcPriceLevel and get('util.appraiser.manifest.color')
		...
		if counts and #counts>0 then
			...
			for _,count in ipairs(counts) do
				...
				local r,g,b,_
				if colored then
					_, _, r,g,b = CalcPriceLevel(itemLink, 1, avgBid, avgBO)
				end
	...
	function frame.UpdatePricing()
		if not frame.salebox.sig then return end
		local link = AucAdvanced.API.GetLinkFromSig(frame.salebox.sig)
	...
	function frame.UpdateDisplay()
		...
		local testmaxbid = false -- used when testing for individual stack bid price over MAXIMUM_BID_PRICE
			...
			if frame.salebox.stacksize > 1 then
				...
				if (curNumber >= -2 and curNumber < 0) then
					...
					if (maxStax > 0) then
						...
						if bidVal ~= buyVal and bidVal > MAXIMUM_BID_PRICE then
							testmaxbid = true
						end
					end
		...
		elseif (curBuy > 0 and curBid > curBuy) then
			...
			canAuction = false
		elseif testmaxbid then -- set by individual tests above
			frame.salebox.warn:SetText("Bid price must not be over the maximum")
			canAuction = false
...
--		frame.DirectSelect(link)
	...
	function frame.PostBySig(sig, dryRun, singleclick)
		local link, itemName = AucAdvanced.API.GetLinkFromSig(sig)
		...
		local itemBuy, itemBid, _, _, _, _, stack, number, rawduration = lib.GetPrice(link, nil, true)
		...
		elseif (number ~= -1) and (stack > total) then
			...
			return
		elseif itemBid ~= itemBuy and itemBid * stack > MAXIMUM_BID_PRICE and (total >= stack or itemBid * total > MAXIMUM_BID_PRICE) then
			aucPrint(("Skipping %s: Bid price is over maximum"):format(link))
			return
		end
	...
	frame.toggleManifest = CreateFrame("Button", nil, frame, "UIPanelButtonTemplate")
	frame.toggleManifest:SetSize(90, 21)
	...
	frame.itembox.showAuctions = CreateFrame("CheckButton", "Auc_Util_Appraiser_ShowAuctions", frame.itembox, "OptionsSmallCheckButtonTemplate")
	...
	frame.itembox.showHidden = CreateFrame("CheckButton", "Auc_Util_Appraiser_ShowHidden", frame.itembox, "OptionsSmallCheckButtonTemplate")
	...
	frame.salebox.stack:SetObeyStepOnDrag(true)
	frame.salebox.stack:SetValue(20)
	frame.salebox.stack:SetSize(180, 17)
	...
	frame.salebox.stack.label:SetJustifyV("MIDDLE")
	...
	frame.salebox.number = CreateSlider("AppraiserSaleboxNumber", frame.salebox)
	...
	frame.salebox.number:SetObeyStepOnDrag(true)
	frame.salebox.number:SetValue(1)
	frame.salebox.number:SetSize(180, 17)
	...
	frame.salebox.number.label:SetJustifyV("MIDDLE")
	...
	frame.salebox.numberentry:Hide()
   	...
   	frame.salebox.numberonly = CreateFrame("CheckButton", "AppraiserSaleboxNumberOnly", frame.salebox, "OptionsSmallCheckButtonTemplate")
   ...
   -- Would rather the distance here matched the length of the "All Stacks" text and was recalculated.
	...
	frame.salebox.duration = CreateSlider("AppraiserSaleboxDuration", frame.salebox)
	...
	frame.salebox.duration:SetObeyStepOnDrag(true)
	frame.salebox.duration:SetValue(3)
	frame.salebox.duration:SetSize(80, 17)
	...
	frame.salebox.duration.label:SetJustifyV("MIDDLE")
	...
	frame.salebox.matcher = CreateFrame("CheckButton", "AppraiserSaleboxMatch", frame.salebox, "OptionsSmallCheckButtonTemplate")
	...
	frame.salebox.ignore = CreateFrame("CheckButton", "AppraiserSaleboxIgnore", frame.salebox, "OptionsSmallCheckButtonTemplate")
	...
	frame.salebox.bulk = CreateFrame("CheckButton", "AppraiserSaleboxBulk", frame.salebox, "OptionsSmallCheckButtonTemplate")
	...
	frame.switchToStack = CreateFrame("Button", nil, frame.salebox, "UIPanelButtonTemplate")
	...
	frame.switchToStack:SetSize(100, 16)
	...
	frame.switchToStack2 = CreateFrame("Button", nil, frame.salebox, "UIPanelButtonTemplate")
	...
	frame.switchToStack2:SetSize(100, 16)
	...
	frame.gobatch = CreateFrame("Button", nil, frame, "UIPanelButtonTemplate")
	...
	frame.cancel = CreateFrame("Button", "AucAdvAppraiserCancelButton", frame, "UIPanelButtonTemplate")
	frame.cancel:SetPoint("BOTTOMLEFT", frame, "BOTTOMLEFT", 180, 15)
	frame.cancel:SetSize(22, 18)
			...
			if (not selected) or (not selected[10]) then
				...
				if (not AucAdvancedConfig["users."..Const.PlayerRealm.."."..private.buyselection.seller]) then
					...
					if private.buyselection.minbid then
						local nextbid
						if private.buyselection.curbid and private.buyselection.curbid > 0 then
							nextbid = math.ceil(private.buyselection.curbid*1.05)
						else
							nextbid = private.buyselection.minbid
						end
						frame.imageview.purchase.bid.price:SetText(AucAdvanced.Coins(nextbid, true))
						if nextbid <= MAXIMUM_BID_PRICE then
	...
	function private.onClick(button, row, index)
		if (IsAltKeyDown()) and frame.imageview.sheet.labels[index]:GetText() == "Seller" then
			...
			if PromptSellerIgnore then
				PromptSellerIgnore(seller, frame.imageview.sheet.panel, "TOPLEFT", button, "BOTTOM")
			end
		end
	end
	...
	frame.imageview.purchase.buy = CreateFrame("Button", nil, frame.imageview.purchase, "UIPanelButtonTemplate")
	frame.imageview.purchase.buy:SetPoint("TOPLEFT", frame.imageview.purchase, "TOPLEFT", 5, 0)
	frame.imageview.purchase.buy:SetSize(30, 21)
	...
	frame.imageview.purchase.bid = CreateFrame("Button", nil, frame.imageview.purchase, "UIPanelButtonTemplate")
	frame.imageview.purchase.bid:SetPoint("TOPLEFT", frame.imageview.purchase.buy, "TOPLEFT", 120, 0)
	frame.imageview.purchase.bid:SetSize(30, 21)
