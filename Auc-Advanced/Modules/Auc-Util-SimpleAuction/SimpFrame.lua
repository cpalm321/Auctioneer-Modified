...
-- Locals for Container functions which use C_Container in some clients ### hybrid
...
-- In this file we only need up to the 2nd return value; however this is copied from CorePost.lua which uses more returns
...
function private.UpdateDisplay()
	...
	local duration = frame.duration.time:GetSelected()
	...
	if cBid > MAXIMUM_BID_PRICE and cBid ~= cBuy then
		frame.err:SetText("Error: Bid price is more than the maximum allowed")
		flagenable = false
	end
...
function private.IconClicked()
	...
	if CursorHasItem() and frame.CursorItemBag and frame.CursorItemSlot then
		_, size = GetContainerItemInfoSpecial(frame.CursorItemBag, frame.CursorItemSlot)
	end
	frame.CursorItemBag = nil
	frame.CursorItemSlot = nil
	ClearCursor()
...
function private.LoadConfig()
	...
	frame.CurItem.buy = buy
	frame.CurItem.buyper = buy/stack
	frame.CurItem.duration = frame.duration.time:SetSelectedHours(duration) -- this function checks for invalid duration
	frame.CurItem.number = number
	frame.CurItem.stack = stack
	frame.CurItem.match = false
	frame.CurItem.undercut = false
...
function private.ClearSetting()
	...
	local under, match = get("util.simpleauc.auto.undercut"), get("util.simpleauc.auto.match")
	if match then
	...
	frame.duration.time:SetDefault()
	frame.CurItem.duration = frame.duration.time:GetSelected()
	private.UpdatePricing()
end
...
function private.CreateFrames()
	...
	if C_Container and C_Container.PickupContainerItem then
		hooksecurefunc(C_Container, "PickupContainerItem", private.postPickupContainerItemHook)
	else
	...
	local t = {}
	frame.duration.time = t
    if AucAdvanced.Classic == 1 then
        t.intervals = {2, 8, 24}
	else
        t.intervals = {12, 24, 48}
    end

	function t:UpdateChecked()
		for pos, dur in ipairs(self.intervals) do
			self[pos]:SetChecked(dur == self.selected)
		end
	end
	function t:SetSelectedHours(hours)
		-- check hours is a valid duration before using it
		for pos, dur in ipairs(self.intervals) do
			if hours == dur then
				self.selected = hours
				self:UpdateChecked()
		...
		-- hours was not valid; return previous value instead
		return self.selected
	end
	function t:SetDefault()
		local dur = get("util.simpleauc.auto.duration")
		if dur == self:SetSelectedHours(dur) then
			return
		end
		-- if we get here, the saved duration was invalid; substitute a valid value
		dur = self.intervals[3] -- use 3rd duration setting, consistent with elsewhere in AucAdvanced
		set("util.simpleauc.auto.duration", dur, true) -- silent
		self:SetSelectedHours(dur)
	end
	function t:GetSelected()
		return self.selected
	end

	local timeOnClick = function (obj)
		frame.CurItem.valuechanged = true
		t:SetSelectedHours(obj.dur)
	end
	for pos, dur in ipairs(t.intervals) do
		t[pos] = CreateFrame("CheckButton", "AucAdvSimpFrameDuration"..dur, frame.duration, "OptionsSmallCheckButtonTemplate")
		...
		t[pos].dur = dur
		t[pos]:SetScript("OnClick", timeOnClick)
	end
	t:SetDefault()
	...
	function frame.options:AddOption(option, text)
		local item = CreateFrame("CheckButton", "AucAdvSimpFrameOption_"..option, self, "OptionsSmallCheckButtonTemplate")
		if self.last then
	...
	function frame.ClickBagHook(_,_,self,button)
		...
		local _, size = GetContainerItemInfoSpecial(bag, slot)
		if link then
		...
		if frame.imageview.sheet.prevselected ~= frame.imageview.sheet.selected then
			...
			if (not selected) or (not selected[10]) then
				...
				if (not AucAdvancedConfig["users."..Const.PlayerRealm.."."..private.buyselection.seller]) then
					...
					if private.buyselection.minbid and private.buyselection.minbid <= MAXIMUM_BID_PRICE then
						frame.bid:Enable()
					else
						frame.bid:Disable()
					end
	...
	frame.scanbutton:SetSize(100, 15)
	frame.scanbutton:SetParent(AuctionFrameBrowse)
	frame.scanbutton:SetPoint("TOPRIGHT", AuctionFrameBrowse, "TOPRIGHT", -57, -17)
	...
	frame.scanbutton:SetScript("OnEnter", function(self)
		GameTooltip:SetOwner(self, "ANCHOR_BOTTOMRIGHT")
		GameTooltip:SetText("Start Auctioneer Scan")
	end)
	frame.scanbutton:SetScript("OnLeave", function() GameTooltip:Hide() end)
	...
	frame.bid = CreateFrame("Button", nil, frame, "UIPanelButtonTemplate")
	frame.bid:SetPoint("TOPLEFT", frame.refresh, "TOPRIGHT", 1, 0)
	frame.bid:SetSize(80, 21)
